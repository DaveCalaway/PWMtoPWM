...
void loop() {
  ...
  pwm_out = map(EMA_S_pulse, 0, 2040, 0, 100);

  if (pwm_out > 5) { // LASER ON
    if (analogRead(button_pin) == 0) { // reset
      delay(15);
      myEnc.write(0);
    }
    else { // Read Encoder
      ...
      if (newPosition != oldPosition) {
        // 0 < pwm_out + newPosition < 99
        if ( pwm_out + newPosition <= 99 && pwm_out + newPosition > 0) {
          analogWrite(pwm_out_pin, pwm_out + newPosition);
        }
        else {
          if ( pwm_out + newPosition > 99 ) {
            newPosition = 100 - pwm_out; // calculate the max/min value can be displayed
            myEnc.write(newPosition);
          }
          else { // pwm_out + (-)newPosition < 0
            newPosition = 100 - pwm_out; // calculate the max/min value can be displayed
